...
def get_random_proxy():
    proxies = Proxy.objects.all(
        filter={"country": "US", "is_working": True}
    )  # Assuming you are using Django ORM and have a Proxy model
...
def get_html(url):
    try:
        r = requests.get(url, headers=get_headers(url), proxies=get_random_proxy())
