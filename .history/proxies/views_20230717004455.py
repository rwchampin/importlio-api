...
class ProxyViewSet(viewsets.ModelViewSet):
    ...
    try:
        instance = Proxy.objects.filter(ip=proxy_data["ip"], port=proxy_data["port"]).first()
        if instance:
            serializer = self.get_serializer(instance, data=proxy_data, partial=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
            return Response(serializer.data, status=200)  # Success, updated the instance
        else:
            serializer = self.get_serializer(data=proxy_data)
            serializer.is_valid(raise_exception=True)
            serializer.save()
            return Response(serializer.data, status=201)  # Success, created a new instance
